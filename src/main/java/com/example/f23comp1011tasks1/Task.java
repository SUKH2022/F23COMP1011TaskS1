...
public class Task {
    ...
    public Task(String title, String description, LocalDate dueDate, int severity, User user) {
        ...
        taskID =-1; //set it to be invalid, the program will get the proper one from the DB
        setTitle(title);
...
// ctrl+b to go to that method
    ...
    public void setDueDate(LocalDate dueDate) {
        if (status== Status.DONE || status== Status.INPROGRESS)
            ...
            throw new IllegalArgumentException("due date must be in the the future");
    }
    ...
    public void setSeverity(int severity) {
        if (severity>=1 && severity<=3)
            ...
            throw new IllegalArgumentException("severity must be in teh range 1-3");
    }
